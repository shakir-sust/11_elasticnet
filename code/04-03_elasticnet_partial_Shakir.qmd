...
First, let's specify:  
  - the **type of model** we want to train [e.g., random forest]  
...
Elastic net **hyperparameters**:  
  ...
  - **mixture**: 0 (ridge) to 1 (lasso) ["mixture" can be anywhere in between 0 to 1]   
...
enet_spec  <- 
  ...
  linear_reg(penalty = tune(), #The 2 hyperparameters are: "penalty = ", "mixture = "
             mixture = tune() #"tune()" function fine-tunes "penalty" and "mixture" hyperparameters for us
...
resampling_foldcv <- vfold_cv(weather_train, #we are giving only the training set (not the whole dataset)
                              v = 10 #v = 10: number of folds that we wannd do #10 is a prettey common number of folds
                              )
...
final_fit %>%
  collect_predictions() #to collect the predictions of the model that was used on the entire training set but getting the predictions on the test set, which we need to assess the model predictive ability
...
We would expect our model to make better predictions on the training set because it was trained on that. So, it is expected that the metrics on the trained set to be better than the test set, because when we trained the model, it was unaware of the test set data (i.e., it had not seen the data, so it's new information for the model). Since the trained model had not seen the data on the test set, we use the fit metrics on the *test set* to assess performance for the predictive ability of the model.
...
final_fit %>%
  ...
  geom_point() +
  geom_abline() + #will come in exam as fill in the blanks #geom_abline(): to add one-to-one line (black line) #something that is very informative in a predicted vs. observed plot is to have a one-to-one line
...
```
The Variable importance plot shows the importance of different variables in our model, and whether they had a positive or negative effect on predicting strength. Variable importance plot is also a common plot to see in elastic net machine learning model.
